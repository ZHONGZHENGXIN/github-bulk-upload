...
const NetworkStatus: React.FC<NetworkStatusProps> = ({
  ...
  const { isOnline, pendingCount, cacheCount, syncInProgress, manualSync } = useOfflineContext()
      ...
      <>
        ...
        >
          <div className={`${getStatusColor()}`}>
            <StatusIcon isOnline={isOnline} pendingCount={pendingCount} syncInProgress={syncInProgress} />
          </div>
    ...
    <div className={`bg-white rounded-lg shadow-sm border p-4 ${className}`}>
      ...
      <div className="space-y-2 text-sm text-gray-600">
        ...
        {pendingCount > 0 && (
          <div className="flex justify-between">
            <span>待同步数量</span>
            <span className="font-medium text-orange-600">{pendingCount} 项</span>
          </div>
        )}

        <div className="flex justify-between">
          <span>缓存数据：</span>
          <span className="font-medium text-gray-700">{cacheCount} 项</span>
        </div>
      </div>
...
const StatusIcon: React.FC<{ isOnline: boolean; pendingCount: number; syncInProgress: boolean }> = ({
  isOnline,
  pendingCount,
  syncInProgress,
}) => {
  if (!isOnline) return <WifiSlashIcon className="h-5 w-5" />
  if (syncInProgress) return <CloudArrowUpIcon className="h-5 w-5 animate-pulse" />
  if (pendingCount > 0) return <ExclamationTriangleIcon className="h-5 w-5" />
